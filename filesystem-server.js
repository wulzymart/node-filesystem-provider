  ...
  if (filenames.indexOf(name) == -1) {
    ...
    for (var i = 0; i < filenames.length; i++) {
      if (filenames[i] === name) {
        if (
          !isFile &&
          fs.lstatSync(directory + "/" + filenames[i]).isDirectory()
        ) {
          return true;
        } else if (
          isFile &&
...
function renameFolder(req, res) {
  ...
  var newName =
    req.body.newName.split("/")[req.body.newName.split("/").length - 1];
  const resolvedOldDirectoryPath = path.resolve(
    contentRootPath + req.body.data[0].filterPath,
    oldName
  );
  const resolvedNewDirectoryPath = path.resolve(
    contentRootPath + req.body.data[0].filterPath,
    newName
  );
  const fullOldPath = (
  ...
  const isValidateOldPath =
    fullOldPath == resolvedOldDirectoryPath ? true : false;
  const isValidateNewPath =
    fullNewPath == resolvedNewDirectoryPath ? true : false;
  ...
  if (permission != null && (!permission.read || !permission.write)) {
    ...
    errorMsg.message =
      permission.message !== ""
        ? permission.message
        : getFileName(contentRootPath + req.body.data[0].filterPath + oldName) +
          " is not accessible.  is not accessible. You need permission to perform the write action.";
    errorMsg.code = "401";
    response = { error: errorMsg };
    response = JSON.stringify(response);
    ...
    var sanitizedPath = path
      .normalize(req.body.data[0].filterPath)
      .replace(/^(\.\.[\/\\])+/, "")
      .replace(/\\/g, "/");
    ...
    if (
      ...
      var errorMsg = new Error();
      errorMsg.message =
        "A file or folder with the name " + req.body.name + " already exists.";
      errorMsg.code = "400";
      response = { error: errorMsg };
      ...
      (async () => {
        await FileManagerDirectoryContent(
          req,
          res,
...
function deleteFolder(req, res, contentRootPath) {
  ...
  req.body.data.forEach(function (item) {
    const resolvedPath = path.join(
      contentRootPath + item.filterPath,
      item.name
    );
    const fullPath = (contentRootPath + item.filterPath + item.name).replace(
      /[\\/]/g,
      "/"
    );
  ...
  if (!permissionDenied) {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      var newDirectoryPath = path.join(
        contentRootPath + req.body.data[i].filterPath,
    ...
    Promise.all(promiseList).then((data) => {
      data.forEach(function (files) {
        if (
          fs
            .lstatSync(
              path.join(contentRootPath + files.filterPath, files.name)
            )
            .isFile()
        ) {
          fs.unlinkSync(
          ...
          const deleteFolderPath = path.join(
            contentRootPath + files.filterPath,
            files.name
          );
          const sanitizedPath = path
            .normalize(deleteFolderPath)
            .replace(/^(\.\.[\/\\])+/, "");
...
function createFolder(req, res, filepath, contentRootPath) {
  ...
  const fullPath = (contentRootPath + req.body.path + req.body.name).replace(
    /\//g,
    "\\\\"
  );
  const isValidatePath = fullPath == resolvedPath ? true : false;
  ...
  var pathPermission = getPathPermission(
    req.path,
    false,
  ...
  if (
    pathPermission != null &&
    (!pathPermission.read || !pathPermission.writeContents)
  ) {
    var errorMsg = new Error();
    errorMsg.message =
      permission.message !== ""
        ? permission.message
        : req.body.data[0].name +
          " is not accessible. You need permission to perform the writeContents action.";
    errorMsg.code = "401";
    response = { error: errorMsg };
    response = JSON.stringify(response);
    res.setHeader("Content-Type", "application/json");
    ...
    if (fs.existsSync(newDirectoryPath)) {
      ...
      (async () => {
        await FileManagerDirectoryContent(req, res, newDirectoryPath).then(
          (data) => {
            response = { files: data };
            response = JSON.stringify(response);
            res.setHeader("Content-Type", "application/json");
...
function checkForMultipleLocations(req, contentRootPath) {
  ...
  if (!isMultipleLocation) {
    location =
      contentRootPath.split("/")[contentRootPath.split("/").length - 1] +
      location.substr(0, location.length - 2);
  }
...
function getFileDetails(req, res, contentRootPath, filterPath) {
  ...
  var sanitizedPathName = path
    .normalize(req.body.names[0])
    .replace(/^(\.\.[\/\\])+/, "")
    .replace(/\\/g, "/");
  var sanitizedPath = path
    .normalize(req.body.path)
  ...
  if (req.body.names.length == 1) {
    fileDetails(
      req,
      res,
      contentRootPath + (isNamesAvailable ? sanitizedPathName : "")
    ).then((data) => {
      if (!data.isFile) {
        getFolderSize(
          req,
      ...
      if (filterPath == "") {
        ...
        data.location = path.join(
          (rootName = path.basename(contentRootPath + sanitizedPath)),
          filterPath,
          req.body.names[0]
        );
      }
...
function checkForFileUpdate(fromPath, toPath, item, contentRootPath, req) {
  ...
  var sanitizedTargetPath = path
    .normalize(req.body.targetPath)
    .replace(/^(\.\.[\/\\])+/, "")
    .replace(/\\/g, "/");
  if (fromPath == toPath) {
    if (
      checkForDuplicates(
        contentRootPath + sanitizedTargetPath,
        name,
        item.isFile
      )
    ) {
    ...
    if (
      req.body.renameFiles.length > 0 &&
      req.body.renameFiles.indexOf(item.name) >= 0
    ) {
      updateCopyName(
        contentRootPath + sanitizedTargetPath,
        name,
...
function CopyFiles(req, res, contentRootPath) {
  ...
  req.body.data.forEach(function (item) {
    ...
    var fileAccessDenied =
      permission != null && (!permission.read || !permission.copy);
    var pathAccessDenied =
      pathPermission != null &&
      (!pathPermission.read || !pathPermission.writeContents);
    if (fileAccessDenied || pathAccessDenied) {
      permissionDenied = true;
      var errorMsg = new Error();
      errorMsg.message = fileAccessDenied
        ? permission.message !== ""
          ? permission.message
          : item.name +
            " is not accessible. You need permission to perform the copy action."
...
function MoveFiles(req, res, contentRootPath) {
  ...
  req.body.data.forEach(function (item) {
    ...
    var fileAccessDenied =
      permission != null && (!permission.read || !permission.write);
    var pathAccessDenied =
      pathPermission != null &&
      (!pathPermission.read || !pathPermission.writeContents);
    if (fileAccessDenied || pathAccessDenied) {
...
function hasPermission(rule) {
  return rule == undefined || rule == null || rule == Permission.Allow
    ? true
    : false;
}
...
function updateRules(filePermission, accessRule) {
  filePermission.download =
    hasPermission(accessRule.read) && hasPermission(accessRule.download);
  filePermission.write =
    hasPermission(accessRule.read) && hasPermission(accessRule.write);
  filePermission.writeContents =
    hasPermission(accessRule.read) && hasPermission(accessRule.writeContents);
  filePermission.copy =
    hasPermission(accessRule.read) && hasPermission(accessRule.copy);
  filePermission.read = hasPermission(accessRule.read);
  filePermission.upload =
    hasPermission(accessRule.read) && hasPermission(accessRule.upload);
  filePermission.message = getMessage(accessRule);
  return filePermission;
}

function getPathPermission(
  path,
  isFile,
  name,
  filepath,
    ...
    accessDetails.rules.forEach(function (accessRule) {
      if (isFile && accessRule.isFile) {
        var nameExtension = name
          .substr(name.lastIndexOf("."), name.length - 1)
          .toLowerCase();
        ...
        if (
          ...
          if (accessRule.path.indexOf("*.*") > -1) {
            var parentPath = accessRule.path.substr(
              0,
              accessRule.path.indexOf("*.*")
            );
            if (
              currentPath.indexOf(contentRootPath + parentPath) == 0 ||
              parentPath == ""
            ) {
            ...
            var pathExtension = accessRule.path
              .substr(
                accessRule.path.lastIndexOf("."),
                accessRule.path.length - 1
              )
              .toLowerCase();
            var parentPath = accessRule.path.substr(
              0,
            ...
            if (
              (contentRootPath + parentPath == currentPath ||
                parentPath == "") &&
            ...
            var pathName = accessRule.path
              ...
              .substr(
                accessRule.path.lastIndexOf("/") + 1,
                accessRule.path.length - 1
              );
            var parentPath = accessRule.path.substr(
              0,
              accessRule.path.indexOf(pathName + ".*")
            );
            if (
              (contentRootPath + parentPath == currentPath ||
                parentPath == "") &&
        ...
        if (
          ...
          if (accessRule.path.indexOf("*") > -1) {
            ...
            if (
              (
                ...
                (parentFolderpath[parentFolderpath.length - 1] == "/"
                  ...
                  : "/") +
                name
              ).lastIndexOf(contentRootPath + parentPath) == 0 ||
              parentPath == ""
            ) {
              filePermission = updateRules(filePermission, accessRule);
            }
          } else if (
            path.join(contentRootPath, accessRule.path) ==
              path.join(parentFolderpath, name) ||
            path.join(contentRootPath, accessRule.path) ==
              path.join(parentFolderpath, name + "/")
          ) {
            filePermission = updateRules(filePermission, accessRule);
            ...
            path
              .join(parentFolderpath, name)
              .lastIndexOf(path.join(contentRootPath, accessRule.path)) == 0
          ) {
            filePermission.write = hasPermission(accessRule.writeContents);
            filePermission.writeContents = hasPermission(
              accessRule.writeContents
            );
            filePermission.message = getMessage(accessRule);
          }
        }
...
function replaceRequestParams(req, res) {
  const sanitizedPath = req.body.path
    ? path.normalize(req.body.path).replace(/\\/g, "/")
    : undefined;
...
function getImage(req, res) {
  ...
  const sanitizedPath = path
    ...
    .replace(/\\/g, "/");
  var image =
    sanitizedPath.split("/").length > 1 ? sanitizedPath : "/" + sanitizedPath;
  ...
  var pathPermission = getPermission(
    contentRootPath + image.substr(0, image.lastIndexOf("/")),
    image.substr(image.lastIndexOf("/") + 1, image.length - 1),
    true,
    contentRootPath,
  ...
  if (pathPermission != null && !pathPermission.read) {
    ...
    fs.readFile(contentRootPath + image, function (err, content) {
      if (err) {
        ...
        return res.status(400).send("No such image");
      } else {
...
app.post(
  ...
  function (req, res) {
    try {
      replaceRequestParams(req, res);
      const checkTraversalPath =
        path
          .resolve(contentRootPath + req.body.path)
          .replace(/[\\/]/g, "\\\\") + "\\\\";
      const actualPath = (contentRootPath + req.body.path).replace(
        /\//g,
        "\\\\"
      );
      ...
      var pathPermission =
        req.body.data != null
          ? getPathPermission(
              req.path,
              true,
              JSON.parse(req.body.data).name,
              contentRootPath + req.body.path,
              contentRootPath,
              JSON.parse(req.body.data).filterPath
            )
          : null;
      if (
        pathPermission != null &&
        ...
        if (req.body.action === "save") {
          ...
          if (folders.length > 1) {
            for (var i = 0; i < folders.length - 1; i++) {
              ...
              const isValidatePath =
                fullPath == newDirectoryPath ? true : false;
              if (!isValidatePath) {
                var errorMsg = new Error();
                errorMsg.message = "Access denied for Directory-traversal";
                errorMsg.code = "401";
          ...
          const normalizedPath = path
            ...
            .replace(/^(\.\.[\/\\])+/, "")
            .replace(/\\/g, "/");
          const cancelUploadValue = path
            .normalize(req.body["cancel-upload"])
            .replace(/^(\.\.[\/\\])+/, "")
            .replace(/\\/g, "/");
          if (
            fs.existsSync(
              path.join(contentRootPath, normalizedPath + cancelUploadValue)
            )
          ) {
            fs.unlinkSync(
              path.join(contentRootPath, normalizedPath + cancelUploadValue)
    ...
    downloadObj.data.forEach(function (item) {
      ...
      if (permission != null && (!permission.read || !permission.download)) {
        ...
        errorMsg.message =
          permission.message !== ""
            ? permission.message
            : getFileName(contentRootPath + item.filterPath + item.name) +
              " is not accessible. You need permission to perform the download action.";
        errorMsg.code = "401";
        response = { error: errorMsg };
        response = JSON.stringify(response);
        res.setH("Content-Type", "application/json");
        return res.json(response);
      }
    });
    if (!permissionDenied) {
      if (downloadObj.names.length === 1 && downloadObj.data[0].isFile) {
        ...
        downloadObj.data.forEach(function (item) {
          ...
          if (item.isFile) {
            archive.file(contentRootPath + item.filterPath + item.name, {
              name: item.name,
            });
          } else {
            archive.directory(
              contentRootPath + item.filterPath + item.name + "/",
              item.name
            );
          }
        });
        ...
        output.on("close", function () {
          ...
          res.set({
            "Content-disposition":
              "attachment; filename=Files.zip; filename*=UTF-8",
...
function handleInit(req, res) {
  ...
  function getRules() {
    ...
    if (!fs.existsSync(accessRuleFile)) {
      return null;
    }
  ...
  function addSearchList(filename, contentRootPath, fileList, files, index) {
    ...
    cwd.filterPath = filename
      .substr(contentRootPath.length, filename.length)
      .replace(files[index], "");
    ...
    var permission = parentsHavePermission(
      filename,
      contentRootPath,
      cwd.isFile,
      cwd.name,
  ...
  function parentsHavePermission(
    filepath,
    contentRootPath,
    isFile,
    name,
    ...
    var parentPath = filepath
      .substr(contentRootPath.length, filepath.length - 1)
      .replace(/\\/g, "/");
    parentPath =
      parentPath.substr(0, parentPath.indexOf(name)) + (isFile ? "" : "/");
  ...
  function checkForSearchResult(
    ...
    var isAddable = false;
    if (
      searchString.substr(0, 1) == "*" &&
      searchString.substr(searchString.length - 1, 1) == "*"
    ) {
      if (
        casesensitive
  ...
  if (req.body.action === "search") {
    const resolvedPath =
      path.resolve(contentRootPath + req.body.path).replace(/[\\/]/g, "\\\\") +
      "\\\\";
    ...
    (async () => {
      const tes = await FileManagerDirectoryContent(
        req,
        res,
        contentRootPath +
          path
        ...
        errorMsg.message =
          permission.message !== ""
            ...
            : "'" +
              getFileName(
                contentRootPath +
                  req.body.path.substring(0, req.body.path.length - 1)
              ) +
              "' is not accessible. You need permission to perform the read action.";
        errorMsg.code = "401";
        response = { error: errorMsg };
        response = JSON.stringify(response);
        res.setHeader("Content-Type", "application/json");
  ...
  function ReadDirectories(file) {
    ...
    function stats(file) {
      return new Promise((resolve, reject) => {
        fs.stat(file, (err, cwd) => {
          ...
          cwd.filterPath = getRelativePath(
            contentRootPath,
            contentRootPath + req.body.path,
            req
          );
          ...
          cwd.permission = getPermission(
            contentRootPath + req.body.path + cwd.name,
            cwd.name,
            cwd.isFile,
            contentRootPath,
            cwd.filterPath
    ...
    for (var i = 0; i < file.length; i++) {
      const sanitizedPath = path
        ...
        .replace(/\\/g, "/");
      promiseList.push(
        stats(path.join(contentRootPath + sanitizedPath, file[i]))
      );
    }
  ...
  if (req.body.action == "read") {
    (async () => {
      ...
      const sanitizedPath = path
        ...
        .replace(/^(\.\.[\/\\])+/, "")
        .replace(/\\/g, "/");
      const cwdFiles = await FileManagerDirectoryContent(
        req,
        res,
        contentRootPath + sanitizedPath
      );
      cwdFiles.name =
        req.body.path == "/"
          ? (rootName = path.basename(contentRootPath + sanitizedPath))
          : path.basename(contentRootPath + sanitizedPath);
      var response = {};
      if (!isValidatePath) {
        var errorMsg = new Error();
        errorMsg.message = "Access denied for Directory-traversal.";
